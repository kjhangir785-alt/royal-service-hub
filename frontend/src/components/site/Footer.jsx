...
export default function Footer() {
  ...
  const hoursText = (business.hours || []).map((h) => `${h.day}: ${h.time}`);
  return (
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 md:grid-cols-2 md:px-8 lg:grid-cols-4 lg:py-24">
        ...
        <div>
          ...
          <p className="mt-4 flex gap-3 font-body text-sm text-white/60">
            ...
            <span>{hoursText.map((t, i) => (<span key={i} className="block">{t}</span>))}</span>
          </p>
        </div>
        ...
        <div>
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            <a href={`https://wa.me/${business.whatsapp || business.phoneRaw}`} target="_blank" rel="noopener noreferrer" data-testid="footer-whatsapp" className="flex items-center gap-3 font-body text-sm text-white/80 transition-colors hover:text-[#d4af37]">
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-6 md:flex-row md:px-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/admin/login" data-testid="footer-admin-link" className="font-body text-xs text-white/25 transition-colors hover:text-[#d4af37]">Owner Login</Link>
          </div>
        </div>
      </div>
    </footer>
